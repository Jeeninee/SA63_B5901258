...
func (pq *ProvinceQuery) QuerySubdistrict() *SubdistrictQuery {
	query := &SubdistrictQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProvinceQuery) QueryStudent() *StudentQuery {
...
//  WithDistrict tells the query-builder to eager-loads the nodes that are connected to
// the "district" edge. The optional arguments used to configure the query builder of the edge.
...
//  WithSubdistrict tells the query-builder to eager-loads the nodes that are connected to
// the "subdistrict" edge. The optional arguments used to configure the query builder of the edge.
func (pq *ProvinceQuery) WithSubdistrict(opts ...func(*SubdistrictQuery)) *ProvinceQuery {
	query := &SubdistrictQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withSubdistrict = query
	return pq
}

//  WithStudent tells the query-builder to eager-loads the nodes that are connected to
// the "student" edge. The optional arguments used to configure the query builder of the edge.
func (pq *ProvinceQuery) WithStudent(opts ...func(*StudentQuery)) *ProvinceQuery {
	query := &StudentQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProvinceQuery) sqlAll(ctx context.Context) ([]*Province, error) {
	var (
		...
		loadedTypes = [3]bool{
			pq.withDistrict != nil,
			pq.withSubdistrict != nil,
			pq.withStudent != nil,
		}
	)
	if pq.withDistrict != nil || pq.withSubdistrict != nil || pq.withStudent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withDistrict; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "district" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withSubdistrict; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Province)
		for i := range nodes {
			if fk := nodes[i].subdistrict; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "subdistrict" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Subdistrict = n
			}
		}
	}
	...
	if query := pq.withStudent; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "student" returned %v`, n.ID)
			}
			for i := range nodes {
